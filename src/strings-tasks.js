...
function extractNameFromTemplate(value) {
  const commaPos = value.indexOf(',') + 2;
  const exclamationPos = value.indexOf('!');

  return value.slice(commaPos, exclamationPos).trim();
}
...
function encodeToRot13(str) {
  ...
  const code = chars.map((char) => {
    if (/[a-zA-Z]/.test(char)) {
      ...
      return String.fromCharCode(start + ((charCode - start + offset) % 26));
    }
    return char;
  });
  return code.join('');
